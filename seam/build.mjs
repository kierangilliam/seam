// Original Author https://github.com/lpil
// Gleam build.js version:2021-09-12

import { exec as callbackExec } from "child_process";
import {
	access, copyFile, mkdir, readdir, readFile, rm,
	stat
} from "fs/promises";
...
export async function build(name) {
	let { gleamDependencies } = JSON.parse(
		await readFile("./package.json")
	);

	if (!gleamDependencies) {
		throw new Error('Define `gleamDependencies` on your `package.json`')
	}
...
async function copyJs(name, dir) {
	...
	files.map(async (file) => {
		if (file.endsWith(".js")) {
			console.log('copy file', join(inDir, file), join(out, file))
...
// 	console.log(join(outDir(name), "main.js"))
// 	return main();
// }
